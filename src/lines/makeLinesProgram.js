...
void main() {
  gl_FragColor = vColor;
  // gl_FragColor = vec4(vColor.abg, 1.0);
}
`;
...
function makeLineProgram(gl, wireCollection, drawTriangles) {
  ...
  const bpe = 4;
  const drawType = drawTriangles ? gl.TRIANGLES : gl.LINES;
  ...
  function draw(transform, count, screen) {
    ...
    gl.bufferData(gl.ARRAY_BUFFER, data, gl.STATIC_DRAW);
    gl.vertexAttribPointer(locations.attributes.aPosition, 2, gl.FLOAT, false, bpe * 3, 0)
    gl.vertexAttribPointer(locations.attributes.aColor, 4, gl.UNSIGNED_BYTE, true, bpe * 3, 2 * bpe)
